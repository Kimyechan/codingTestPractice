...
#
#     for idx, page in enumerate(pages):
#         p1 = re.compile('<meta property="og:url" content="(.*?)"/>')
#         metaContent = p1.findall(page)
#
#         p2 = re.compile('<a href="(.*?)">')
#         links = p2.findall(page)
#
#         count = re.sub('[^a-zA-Z]', ' ', page).lower().split().count(word.lower())
#
#         basicScore[metaContent[0]] = count
#         linkList[metaContent[0]] = links
#         pageIndex[metaContent[0]] = idx
#
#     totalScore = deepcopy(basicScore)
#     for key, links in linkList.items():
#         for link in links:
#             if basicScore.get(link) is None:
#                 continue
#             totalScore[link] += basicScore[key] / len(links)
#
#     result = []
#     for key, score in totalScore.items():
#         result.append((pageIndex[key], score))
#     result.sort(key=lambda x: (-x[1], x[0]))
#
#     print(result)
#     return result[0][0]
